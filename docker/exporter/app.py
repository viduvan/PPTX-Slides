...
@app.post("/export-pdf", response_model=ExportPdfResponse)
async def export_pdf(req: ExportPdfRequest):
    ...
    job_dir = OUTPUT_DIR / req.job_id
    job_dir.mkdir(parents=True, exist_ok=True)
    pdf_path = job_dir / f"presentation_{req.job_id}.pdf"
    ...
    try:
        ...
        result = subprocess.run(
            [lo_cmd, "--headless", "--convert-to", "pdf",
             "--outdir", str(job_dir), str(pptx_path)],
            capture_output=True, text=True, timeout=120,
        )
        if result.returncode != 0:
            logger.error(f"LibreOffice failed: {result.stderr}")
        ...
        # The output file is named {pptx_path.stem}.pdf inside job_dir.
        expected_pdf = job_dir / f"{pptx_path.stem}.pdf"
        if not expected_pdf.exists():
            generated_pdfs = list(job_dir.glob("*.pdf"))
            if generated_pdfs:
                expected_pdf = generated_pdfs[0]
            else:
                raise HTTPException(status_code=500, detail="PDF was not generated by LibreOffice")

        # Rename to target pdf_path if it's different
        if expected_pdf != pdf_path:
            shutil.move(str(expected_pdf), str(pdf_path))

        logger.info(f"[{req.job_id}] Successfully exported PDF: {pdf_path}")
        return ExportPdfResponse(
            job_id=req.job_id,
            pdf_path=str(pdf_path),
        )
    except subprocess.TimeoutExpired:
        logger.error(f"PDF export timed out for {req.job_id}")
...
def _resolve_layout(slide_data: dict, layouts_spec: list, idx: int, total: int) -> str:
    ...
    if idx < len(layouts_spec):
        ...
        if isinstance(layout_name, dict):
            layout_name = layout_name.get("layout") or layout_name.get("layout_name") or layout_name.get("type") or ""
